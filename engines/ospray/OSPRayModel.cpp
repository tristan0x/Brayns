...
void OSPRayModel::_commitSpheres(const size_t materialId) {
    ...
    auto data = allocateVectorData(_geometries->_spheres.at(materialId), OSP_FLOAT,
...
void OSPRayModel::_commitCylinders(const size_t materialId) {
    ...
    auto data = allocateVectorData(_geometries->_cylinders.at(materialId), OSP_FLOAT,
                                   _memoryManagementFlags);
...
void OSPRayModel::_commitCones(const size_t materialId) {
    ...
    auto data = allocateVectorData(_geometries->_cones.at(materialId), OSP_FLOAT,
                                   _memoryManagementFlags);
...
void OSPRayModel::_commitMeshes(const size_t materialId) {
    ...
    OSPData indices = allocateVectorData(trianglesMesh.indices, OSP_INT3, _memoryManagementFlags);
    ...
    if (!trianglesMesh.textureCoordinates.empty()) {
        OSPData texCoords = allocateVectorData(trianglesMesh.textureCoordinates, OSP_FLOAT2,
                                               _memoryManagementFlags);
...
void OSPRayModel::_commitSDFGeometries() {
    ...
    for (size_t geomI = 0; geomI < numGeoms; geomI++) {
        ...
        if (!neighsI.empty()) {
            ...
            _geometries->_sdf.neighboursFlat.insert(std::end(_geometries->_sdf.neighboursFlat),
    ...
    auto neighbourData = allocateVectorData(_geometries->_sdf.neighboursFlat, OSP_ULONG,
                                            _memoryManagementFlags);
    ...
    for (const auto& mat: _materials) {
        ...
        auto data = allocateVectorData(_geometries->_sdf.geometryIndices[materialId], OSP_ULONG,
                                       _memoryManagementFlags);
...
void OSPRayModel::_setBVHFlags() {
    ...
    osphelper::set(_primaryModel, "robustMode", static_cast<int>(_bvhFlags.count(BVHFlag::robust)));
}
...
void OSPRayModel::commitMaterials(const std::string& renderer) {
    ...
    if (_renderer != renderer) {
        ...
        for (auto& map: {_ospSpheres, _ospCylinders, _ospCones, _ospMeshes, _ospStreamlines,
                         _ospSDFGeometries}) {
            ...
            while (matIt != _materials.end() && geomIt != map.end()) {
                ...
                if (matIt->first != geomIt->first) {
                    BRAYNS_ERROR << "Material for geometry missing" << std::endl;
                    ++geomIt;
                    continue;
                }
...
SharedDataVolumePtr OSPRayModel::createSharedDataVolume(const Vector3ui& dimensions,
                                                        const Vector3f& spacing,
                                                        const DataType type) const {
    return std::make_shared<OSPRaySharedDataVolume>(dimensions, spacing, type, _volumeParameters,
...
BrickedVolumePtr OSPRayModel::createBrickedVolume(const Vector3ui& dimensions,
                                                  const Vector3f& spacing,
                                                  const DataType type) const {
    return std::make_shared<OSPRayBrickedVolume>(dimensions, spacing, type, _volumeParameters,
                                                 _ospTransferFunction);
}
