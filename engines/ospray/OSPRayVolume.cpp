...
namespace brayns {
OSPRayVolume::OSPRayVolume(const Vector3ui& dimensions,
...
OSPRaySharedDataVolume::OSPRaySharedDataVolume(const Vector3ui& dimensions,
                                               const Vector3f& spacing,
                                               const DataType type,
                                               const VolumeParameters& params,
                                               OSPTransferFunction transferFunction)
...
void OSPRayBrickedVolume::setBrick(const void* data,
...
void OSPRaySharedDataVolume::setVoxels(const void* voxels) {
    OSPData data = ospNewData(glm::compMul(SharedDataVolume::_dimensions), _ospType, voxels,
...
void OSPRayVolume::commit() {
    if (_parameters.isModified()) {
        ...
        osphelper::set(_volume, "adaptiveSampling", _parameters.getAdaptiveSampling() ? 1 : 0);
        osphelper::set(_volume, "singleShade", _parameters.getSingleShade() ? 1 : 0);
        osphelper::set(_volume, "preIntegration", _parameters.getPreIntegration() ? 1 : 0);
